...
#define B_TRANSLATION_CONTEXT "BrainWindow"
...
TheBrainWindow::TheBrainWindow(BRect cTWindowRect,const char* argv)
	:
	BWindow(cTWindowRect, B_TRANSLATE_SYSTEM_NAME("TheBrain"), B_TITLED_WINDOW,
		B_AUTO_UPDATE_SIZE_LIMITS | B_NOT_V_RESIZABLE | B_NOT_ZOOMABLE)
{
debug_printf("TheBrainWindow {constructor}\n");
//debug_printf("TheBrainWindow {constructor} string is %s\n", *argv);
debug_printf("TheBrainWindow {constructor} string is non ptr %s\n", argv);
//if (*command ) {
//	debug_printf("TheBrainWindow {constructor} command is not null\n");
//}
	// TextControls
...
void
TheBrainWindow::MessageReceived(BMessage* cTMessage)
{

debug_printf("TheBrainWindow:: MessageReceived\n");
debug_printf("TheBrainWindow:: MessageReceived what = %d\n", cTMessage->what);
debug_printf("TheBrainWindow:: MessageReceived ExecuteOrNot %d\n", ExecuteOrNot);
...
//debug_printf("TheBrainWindow:: MessageReceived command executed\n");
/*BString	fCommand = "a.out";//"pwd"; //"/bin/bash -c ./a.out";
	debug_printf("fCommand %s\n", fCommand.String());
	FILE* fd = popen(fCommand.String(),"r");
	if (fd != NULL) {
	...
	switch (cTMessage->what) {
		case kmsg:
			debug_printf("TheBrainWindow:: MessageReceived DRCT\n");
			executeCompileCommand();
			break;
		case 'CLNN':
		{
...
// this works			const char* message = "eight";
			...
			if (area < 0) {
				debug_printf("TheBrain Could not write area.\n");
			}
			...
			if(result == B_OK){
				...
				debug_printf("TheBrainWindow = error writing to port.\n");
			}
			break;
		}
		case 'PULS':
...
BString
TheBrainWindow::getPath(BMessage* message)
{
	//int32 ref_num;
	entry_ref ref;
...
BString
TheBrainWindow::getFolder(BMessage* message)
{
	entry_ref ref;
	BString text = "Error";
...
void
TheBrainWindow::setPermissions(bool permission)
{
...
void
TheBrainWindow::executeCompileCommand()
{
debug_printf("TheBrainWindow:: executeCompileCommand\n");
	BString cmd = "%command% &";
	/*if (path) {
		cmd = "cd %path% ; %command% &";
		cmd.ReplaceFirst("%path%", pathTextControl->Text());
...
debug_printf("TheBrainWindow:: executeCompileCommand %s\n", cmd.String());
