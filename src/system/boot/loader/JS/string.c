 ...
 */
void *memcpy2(void *__restrict dest, const void *__restrict src, size_t count)
{
 ...
 * @return		Destination location.
 */
void *memset2(void *dest, int val, size_t count)
{
 ...
 */
int memcmp2(const void *p1, const void *p2, size_t count)
{
 ...
 * @return		Length of the string. */
size_t strnlen2(const char *str, size_t count)
{
	size_t ret;
 ...
 *               or to be greater than s2.
 */
int strncasecmp2(const char *s1, const char *s2, size_t count)
{
	unsigned char c1, c2;
...
{
	size_t len = strlen2(what);

	while (*s) {
		if (strncmp2(s, what, len) == 0)
			return (char*)s;
		s++;
	}
 ...
 */
char *strcpy2(char *__restrict dest, const char *__restrict src)
{
	char *d = dest;
 ...
 */
char *strcat2(char *__restrict dest, const char *__restrict src)
{
 ...
 * @return              Pointer to the allocated buffer containing the string.
 */
char *strndup2(const char *src, size_t n)
{
	size_t len;
	char *dup;
...
/** Macro to implement strtoul() and strtoull2(). */
 ...
 */
unsigned long strtoul2(const char *cp, char **endp, unsigned int base)
{
 ...
 * @return		Converted value.
 */
long strtol2(const char *cp, char **endp, unsigned int base)
{
	if (*cp == '-')
		return -strtoul2(cp + 1, endp, base);

	return strtoul2(cp, endp, base);
}
 ...
 * @return		Converted value.
 */
unsigned long long strtoull2(const char *cp, char **endp, unsigned int base)
{
 ...
 * @return		Converted value.
 */
long long strtoll2(const char *cp, char **endp, unsigned int base)
{
	if (*cp == '-')
		return -strtoull2(cp + 1, endp, base);

	return strtoull2(cp, endp, base);
}
...
{
	return vsnprintf2(buf, (size_t)-1, fmt, args);
}
 ...
 */
int snprintf2(char *buf, size_t size, const char *fmt, ...)
{
	va_list args;
	int ret;

	va_start(args, fmt);
	ret = vsnprintf2(buf, size, fmt, args);
	va_end(args);
