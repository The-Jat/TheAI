...
static input_result_t ui_list_input(ui_window_t *window, uint16_t key)
{
	...
	case CONSOLE_KEY_UP:
		...
		while (true) {
			...
			// scroll u if off the visible are. Doing it here means we will  show
			// non-selectable entries even at the top of the list
			if (target < list->offset) {
				list->offset--;
				console_scroll_up(current_console);

				// if not selectable, we're going to scroll over this which means it
				// won't be rendered by the highlight below. Render it unhighlighted.
				if (!selectable) {
...
void ui_list_insert(ui_window_t *window, ui_entry_t *entry, bool selected)
{
  ...
  // make this selected if the first entry is not selectable (can happen if the
  // first thing is a section header)
...
static void ui_textbox_render(ui_entry_t *entry)
{
	...
	avail = CONTENT_WIDTH - strlen2(box->label) - 3;
	len = strlen2(box->value->string);
	if (len > avail) {
		printf(" [");
...
static input_result_t ui_textbox_input(ui_entry_t *entry, uint16_t key)
{
	...
	if (key == '\n') {
		...
		len = strlen2(box->label) + 8;
		title = malloc(len);
		snprintf2(title, len, "Edit '%s'", box->label);
...
static void ui_chooser_render(ui_entry_t *entry)
{
	...
	if (chooser->selected->label) {
		snprintf2(buf, sizeof(buf), "%s", chooser->selected->label);
	} else {
		switch (chooser->value->type) {
		case VALUE_TYPE_INTEGER:
			snprintf2(buf, sizeof(buf), "%" J_PRIu64, chooser->value->integer);
			break;
		case VALUE_TYPE_BOOLEAN:
			snprintf2(buf, sizeof(buf), (chooser->value->boolean) ? "True" : "False");
			break;
		case VALUE_TYPE_STRING:
			snprintf2(buf, sizeof(buf), "%s", chooser->value->string);
			break;
		default:
			unreachable();
		}
	...
	avail = CONTENT_WIDTH - strlen2(chooser->label) - 3;
	len = strlen2(buf);
	if (len > avail) {
		printf(" [");
		for (size_t i = 0; i < avail - 3; i++)
...
static void ui_choice_render(ui_entry_t *entry)
{
	...
	if (choice->label) {
		...
		case VALUE_TYPE_INTEGER:
			printf("%" J_PRIu64, choice->value.integer);
			break;
		case VALUE_TYPE_BOOLEAN:
