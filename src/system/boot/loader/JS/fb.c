...
//i dont have the red blue adn green from the vesa later do it working...
...
static void fb_fillrect(fb_console_out_t *fb, uint32_t x, uint32_t y, uint32_t width, uint32_t height, uint32_t rgb) {
  if (x == 0 && width == current_video_mode->width && (rgb == 0 || rgb == 0xffffff)) {
    ...
    memset2(fb->mapping + (y * current_video_mode->pitch), (uint8_t)rgb, height * current_video_mode->pitch);
    memset2(fb->backbuffer + (y * current_video_mode->pitch), (uint8_t)rgb, height * current_video_mode->pitch);
  } else {
  ...
  uint32_t width, uint32_t height)
{
  ...
  if (dest_x == 0 && src_x == 0 && width == current_video_mode->width) {
    ...
    memmove2(fb->backbuffer + dest_offset, fb->backbuffer + src_offset, height * current_video_mode->pitch);
    memcpy2(fb->mapping + dest_offset, fb->backbuffer + dest_offset, height * current_video_mode->pitch);
  } else {
    /* Copy line by line. */
    for (uint32_t i = 0; i < height; i++) {
      ...
      memmove2(fb->backbuffer + dest_offset, fb->backbuffer + src_offset, width * (current_video_mode->bpp >> 3));
      memcpy2(fb->mapping + dest_offset, fb->backbuffer + dest_offset, width * (current_video_mode->bpp >> 3));
    }
  }
}
...
static void fb_console_set_region(console_out_t *console, const draw_region_t *region) {
  ...
  if (region) {
    ...
    memcpy2(&fb->region, region, sizeof(fb->region));
  } else {
...
static void fb_console_get_region(console_out_t *console, draw_region_t *region) {
  ...
  memcpy2(region, &fb->region, sizeof(*region));
}
...
static void fb_console_scroll_up(console_out_t *console) {
  ...
  memset2(&fb->chars[(fb->region.y * fb->cols) + fb->region.x], 0, fb->region.width * sizeof(*fb->chars));
  fb_fillrect(fb,
...
//dprintf("into the fb_console_init:\n");
  //assert(current_video_mode->type == VIDEO_MODE_LFB);

//own code
//current_video_mode->mem_virt=gKernelArgs.frame_buffer.physical_buffer;
do_video_assignment();
//dprintf("current_video_mode->mem_virt=%lu\n",current_video_mode->mem_virt);
//js code start
//fbmapping2
//js code end
...
//  fb->backbuffer = memory_alloc(size, 0, 0, 0, MEMORY_TYPE_INTERNAL, MEMORY_ALLOC_HIGH, NULL);
 ...
 // fb->chars = memory_alloc(size, 0, 0, 0, MEMORY_TYPE_INTERNAL, MEMORY_ALLOC_HIGH, NULL);
  ...
  // clear the console.
  fb_fillrect(fb, 0, 0, current_video_mode->width, current_video_mode->height, fb_color_table[COLOR_MAGENTA]);
  memset2(fb->chars, 0, fb->cols * fb->rows * sizeof(*fb->chars));
